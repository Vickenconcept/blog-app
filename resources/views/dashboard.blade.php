<x-app-layout>
    ...
    <div class="h-screen flex justify-center items-center">
        <div class="w-full max-w-sm p-4 bg-white border border-gray-200 rounded-lg shadow sm:p-6 md:p-8 ">
            ...
            <form class="space-y-6" action="{{ route('blogs.store') }}" method="post">

                @csrf

                <h5 class="text-xl font-medium text-gray-900 ">Create blog</h5>
                <div>
                ...
                <div>
                    ...
                    <textarea id="content" rows="4" class="bg-gray-50  focus:ring-transparent form-control"   name="content"
                        autocomplete="false">{{ old('content') }}</textarea>

                </div>
                <div>
                    <label for="author" class="  block mb-2 text-sm font-medium text-gray-900 ">
                        Author</label>
                    <input type="author" name="author" id="author" placeholder="describe your blog " value="{{ old('author') }}"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-red-500 focus:border-red-500 block w-full p-2.5 ">
                </div>
